...
### 3）延长时段K线的数据源评估（结论：维持雅虎）

曾评估「让 1h 也包含美股盘前/盘后延长时段」的多种数据源，最终**结论维持雅虎现状**。各方案对比：

| 方案 | 能否含延长时段 | 历史获取方式 | 结论 |
| ---- | ---- | ---- | ---- |
| **Yahoo（当前）** | 1m/5m/15m/30m ✅、**1h/1d ❌** | REST 一次拉取 | ✅ 免费、无限量、已接入 |
| **Alpha Vantage** | ✅ 官方支持 extended_hours | REST | ❌ 免费层 60min 已变付费端点，且免费配额仅 25 次/天，无法支撑数百成分股 |
| **AkShare** | ❌ 无美股小时K线接口 | — | ❌ 免费但根本没有美股小时级K线 |
| **新浪财经** | 实时快照含盘后 ✅、K线序列 ❌ | 实时行情接口可用，K线端点已废弃 | ⚠️ 仅作外部验证接口 |
| **Gate 股票** | ✅ 美股 1h 支持 `"true"` 开启盘前/盘后 | **WebSocket 实时推送**（需常驻订阅累积） | ⚠️ 能补上，但仅覆盖美股，且是流式推送需长期运行，与现有定时模型不匹配 |

**最终决策**：
- 保留雅虎为唯一入库数据源。
- **1m/5m/15m/30m** 已含美股延长时段（4:00–20:00 美东），满足分钟级需求。
- **1h 维持雅虎原生**（盘中 9:30–16:00，含 `15:30–16:00` 收盘bar），不含盘前/盘后。
- 若未来确需「1h 含延长时段」，优先考虑接入 **Gate 股票 `stock.kline` 频道**（`payload: ["1h", symbol, "true"]`，bar 带时段标记 `m: earlyHours/afterHours/open/closed`），但需新增常驻 WebSocket 采集器，且仅覆盖美股。
